...
class Calculation2Controller extends Controller
{
    public function hitung2(Request $request)
    {
        ...
        switch (true) {
            ...
            $filteredInheritance = array_filter([
            ...
            if (!empty($filteredInheritance)) {
                $heir = new Heir([
                    'gender' => $jenisKelamin,
                    'status' => $status,
                    'inheritance' => json_encode($filteredInheritance),
                    'total_inheritance' => $totalWarisan,
                ...
                try {
                    $user->heirs()->save($heir);
                } catch (\Exception $e) {
                    return redirect()->back()->with('error', 'Gagal menyimpan data warisan: ' . $e->getMessage());
                }
            }
            
            // Setelah penyimpanan, kembalikan hasil perhitungan ke tampilan
